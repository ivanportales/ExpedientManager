...
class ScaleSelectTypeTests: XCTestCase {
    ...
    func test_onDutyHidesRestLineStackContainer() {
        ...
        XCTAssertFalse(sut.workDurationTypeTxtField.isEnabled)
        XCTAssertEqual(sut.workDurationTypeTxtField.textColor, UIColor.text)
        XCTAssertTrue(sut.lineSeparatorView.isHidden)
    }
    ...
    func test_restDurationValueChanged() {
        makeSUT()
        
        sut.restDurationTxtField.text = "12"

        XCTAssertEqual(sut.restDuration, 12)
    }
